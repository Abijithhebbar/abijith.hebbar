...
def displayHand(hand):
    ...
    for letter in hand.keys():
        for j in range(hand[letter]):
            string += letter+' ' 
    return string                      # print all on the same line        
#
# Problem #2: Make sure you understand how this function works and what it does!
#
def dealHand(n):
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(hand) > 0:
        ...
        string = input("Enter word, or a . to indicate that you are finished:")
        if string == '.':
            print('Goodbye! Total score: '+ str(totalScore)+' points.')
            break
        else:
            if not isValidWord(string, hand, wordList):
                print('Invalid word, please try again.')
            else:
                wordScore = int(getWordScore(string,n))
                totalScore += wordScore
                print(string +" earned"+ str(wordScore)+ " points.Total:" + str(totalScore)+ " points")
                hand = updateHand(hand, string)
    ...
    while True:
        ...
        if user_input == 'n':
            ...
            if hand == {}:
                ...
                playHand(b_dict,wordList,HAND_SIZE)    
        elif user_input == 'e':
            print('exit game')
            return
        else:
